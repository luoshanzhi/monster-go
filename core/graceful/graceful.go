...
func SignalHandler(file *os.File, callback func(ctx context.Context)) {
	...
	for {
		...
		case syscall.SIGINT, syscall.SIGTERM:
			signal.Stop(ch)
			callback(ctx)
			core.CommonLog.Info("程序退出成功")
			return
		case syscall.SIGUSR2:
			err := reload(file)
			callback(ctx)
			if err != nil {
				core.CommonLog.Info("程序热更新异常", err)
			} else {
				core.CommonLog.Info("程序热更新成功")
			}
			return
		}
	}
